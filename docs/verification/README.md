...
- Cross-Run 修复前的真实失败证据：Run 1–3 完成；Run 4 到达 `40 / 40` 并以 `AgentMaxStepsError` 结束。Run 4 没有执行 pytest、没有触发 Progress Guard intervention，首次 mutation 发生在 step 40；mutation 前有 162 次读取，其中 125 次为重复完整读取，且没有调用 trusted verification。Evidence Preservation 通过。根因已定位为缺少跨 Run pending-failure handoff：Run 3 baseline pytest 已失败，但新建的 Run 4 Progress Guard 从 NORMAL 启动。
- Cross-run repair-control patch：runner 现在从 Run 3 的真实 tracked-command Tool Result 派生最小 pending failure fact，并以该事实 seed Run 4 的通用 Progress Guard；Run 4 最多使用 3 个 initial diagnosis responses，随后必须真实 mutation，ordinary retest failure 仍只开放 1 个 diagnosis response。相同 runtime Guard config 同时初始化 repair telemetry，因此 initial pending、首次无 mutation retest、零 pytest 与 run-end pending 状态保持一致；零 pytest 不再误报 Repair Loop PASS。公开 CLI oracle 也补齐了 programmatic `SystemExit(2)`、file-open 前参数验证与真实 `python -m incident.cli --report` 完整 v2 payload。Hidden evaluator 保持不变。下一次 Real Validation：**Pending**，本轮未执行真实收费模型。
- 最新真实 DeepSeek-V4-Flash 验证（2026-09-02 13:10）：Run 1–3 完成；Run 4 从 Cross-Run pending tracked failure 启动并使用 3 个 initial diagnosis responses，首次 mutation 为 step 5，mutation 前重复完整读取为 0。Run 4 共执行 7 次 tracked pytest、产生 43 次 Progress Guard interventions 和 11 次 failure 后成功 mutation，但在 `40 / 40` 以 `AgentMaxStepsError` 结束，run end 仍保留 pending failed pytest，未到达 diagnostic READY、trusted verification 或 Final。模型把 repair surface 扩张到 `incident/contract.py`，并修改了初始已正确的 `incident/models.py`、`incident/parser.py` 与 `incident/cli.py`，同时破坏 `IncidentStore` API 和 Report 契约；最终 visible 状态为 `13 failed / 13 passed`。本次为 60 次 LLM calls、164 次 Tool calls、内部约 `683.46` 秒（外部约 686 秒），Runtime Target 与 Evidence Preservation PASS，Functional、Coverage 与 Real Validation Process FAIL；结构化报告对应 `.runs/long_running_challenge/20260902_131029_e8fde1b8/`，继续保留为真实失败证据。
- Reduced-complexity benchmark patch：起始 fixture 中 constants severity order、Parser 和基础 CLI 行为现在正确，Parser/CLI tests 继续作为 regression coverage；Store、Service、Serializer 与初始缺失的 Report 仍是 repair targets。README 明确公开 `incident.report.build_report(received, actionable)` 及完整 v2 schema。Run 4 initial diagnosis allowance 保持 3，普通 pytest failure 后 targeted diagnosis allowance 从 1 调整为 2，随后仍由 Host 强制真实 mutation；首 mutation preferred warning target 调整为 `<=5`，production modified coverage 调整为 `>=3` 且 created file 仍要求 `>=1`。四 Runs、1800 行 migration、24,000 字符 Context、Cross-Run Guard、Hidden、Verification、Evidence、controlled errors、truncation 与 300–900 秒 Runtime Target 均未降低。下一次 Real Validation：**Pending**，本轮未执行真实收费模型。
- Final stabilization patch：最终 repair targets 收敛为 `incident/store.py`、`incident/serializer.py` 与 `incident/report.py`。`service.py` 改为 initial-correct，`report.py` 初始存在且仅提供精确 `(received, actionable)` 签名的 `NotImplementedError` stub；新增受保护的 8-case `tests/test_release_repair.py`。Run 3 baseline 与 Run 4 exact tracked retest 使用 focused command `python -B -m pytest tests/test_release_repair.py -q --basetemp=.pytest_tmp`，最终 `verify_workspace` 与 Host recheck 仍运行 full visible suite。通用 `allowed_mutation_paths` 在整个 guarded Run 生命周期 Host-enforce 三文件写入面；越界的既有文件改写、新建 `contract.py` 及 NORMAL 状态下的 late rewrite 均返回结构化 `ProgressGuardBlocked`，不会执行工具或推进 Workspace revision。Hidden Group 1/2/3 evaluator 保持 `0 diff`；Long-Running 不再硬性要求创建文件，`production_files_changed >= 3` 保持不变，`files_created` 仅作信息记录。
- Final stabilization 离线证明：fresh focused baseline 为 `7 failed / 1 passed`，fresh full visible baseline 为 `16 failed / 18 passed`，Parser + Service + 基础 CLI 为 `13 passed`。确定性 reference repair 只修改上述三个 target；同一 workspace 中 focused、full visible、Hidden Group 1/2/3 和 protected-file integrity 全部 PASS，initial-correct production files 未改变。下一次 Real Validation：**Pending**；本轮未执行真实收费模型。
...
- Repository Evidence：初始 23 个文件、34 个 visible tests、10+ distinct files read、至少 3 个 existing production files changed；`incident/report.py` 初始存在为正确签名 stub，created-file 数量不再是 hard requirement。
...
- 运行日志：最新完整 trace 保存在被 Git 忽略的 `.runs/long_running_challenge_20260902_131026.log`，对应 Evidence 为 `.runs/long_running_challenge/20260902_131029_e8fde1b8/`；更早日志和 Evidence 继续保留。后续正式命令仍使用新的时间戳日志，不覆盖历史证据。
